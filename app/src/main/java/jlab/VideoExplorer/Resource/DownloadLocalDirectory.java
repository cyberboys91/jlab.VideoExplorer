                ...
                if (!loaded) {
                    ...
                    for (int i = 0; i < stgPath.size(); i++) {
                        ...
                        if (Utils.existAndMountDir(current)) {
                            loadContentForDir(allVideos, String.format("%s/%s/", current, Environment.DIRECTORY_DOWNLOADS));
                            loadContentForDir(allVideos, String.format("%s/Android/data/%s/files/", current, Utils.getPackageName()));
                        }
                    }
